...
- A publikus backend elérés dedikált, implementációfüggetlen API Gateway rétegen keresztül történik; Traefik vagy Nginx javasolt önmenedzselt megoldás, de nem kötelező termékválasztás.
...
- A külső HTTPS forgalom első belépési pontja dedikált API Gateway, amely TLS terminációt, routingot, request size limiteket, durva rate limitinget és JWT elővalidálást végez.
