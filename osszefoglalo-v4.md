# Community Choice - összefoglaló v4

Generált összefoglaló a publikált dokumentációs források alapján. Létrehozás dátuma: 2026-05-18.

A dokumentum a `docs/src/content/docs/` alatti Starlight oldalak teljes tartalmát fűzi össze, a Starlight oldalsáv (sidebar) szerinti sorrendben.
...
<div class="lp-tech-badges">
  ...
  <span class="lp-badge">Vault / KMS kulcskezelés</span>
...
* A jogosultsági pillanatképben tárolt felhasználóazonosító-hash csak védett kulcskezeléssel képezhető; a hash képzéséhez használt titok nem kerülhet az alkalmazás memóriájába, konfigurációjába vagy naplóiba.
...
Az outbox relay worker olvassa az outbox rekordokat `SELECT ... FOR UPDATE SKIP LOCKED` stratégiával, majd publikálja a `vote-cast-events` tartós streambe. Így ha a broker vagy a hálózat átmenetileg hibázik, a szavazati commit nem vész el, és az audit esemény később pótolható. A kulcskezelő a jogosultsági hash és a szavazói kulcs képzéséhez használt titkokat védi, a PgBouncer pedig az adatbázis-kapcsolatok kontrollját biztosítja.
...
Ez a szolgáltatás felel a jogosult, pszeudonimizált és megváltoztathatatlan szavazatok rögzítéséért. Tartalmazza a Zamunda One klienst, amely a lakcímet és - ha az API támogatja - a lakcím-módosítási dátumot/időbélyeget kéri le, valamint a Jogosultságkezelőt, amely a `campaign_eligibility` pillanatképet ellenőrzi vagy létrehozza. Új pillanatkép létrehozásakor a rendszer a nyers Zamunda One azonosítóból védett hash értéket képez, és csak ezt tárolja. A szolgáltatás a jogosultsági pillanatkép kontextusából pszeudonimizált szavazói kulcsot képez, majd a szavazatot PostgreSQL-ben, csak hozzáfűzhető szavazati tranzakciónaplóban tárolja.
...
- a `campaign_eligibility` tábla determinisztikus felhasználóazonosító-hash mezővel tud ismételt kampány-interakciót felismerni, miközben nyers Zamunda One ID-t nem tárol
